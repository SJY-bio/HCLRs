...
IPF <- dir("D:/ATAC/HCLRs/GSE176078/sc_rds/")
IPF <- sub("\\.rds$", "", IPF)
sp = 1

for (sp in 1:length(IPF)) {
  # Seurat to AnnData
  IPF_seurat = readRDS(paste0("sc_rds/",dir("D:/ATAC/HCLRs/GSE176078/sc_rds/")[sp]))
  ...
  sceasy::convertFormat( IPF_seurat, from="seurat", to="anndata", assay = "RNA", main_layer = "count",
                         outFile=paste0("D:/ATAC/HCLRs/GSE176078/sc_h5ad/",IPF[sp],".h5ad"))
  meta <- IPF_seurat@meta.data
  df_meta <- tibble(
    Cell      = rownames(meta),
    cell_type = meta$cell.type
  )
  fwrite(df_meta,file = paste0("D:/ATAC/HCLRs/GSE176078/sc_h5ad/",IPF[sp],"_meta.txt"),quote = F,sep = "\t")
}
